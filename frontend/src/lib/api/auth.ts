...
export const authApi = {
    ...
    register: (userData: userRegisterDto) =>
        apiCall<ApiResponse<userLoginResBody>>('/api/v1/users/register', {
            method: 'POST',
    ...
    login: (credentials: userLoginReqBody) =>
        apiCall<ApiResponse<userLoginResBody>>('/api/v1/users/login', {
            method: 'POST',
    ...
    // 로그아웃
    logout: () =>
        apiCall<ApiResponse<void>>('/api/v1/users/logout', {
            method: 'DELETE',
        }),

    // 회원 탈퇴
    deleteUser: (userLoginId: string) =>
        apiCall<ApiResponse<void>>(`/api/v1/users?userLoginId=${userLoginId}`, {
            method: 'DELETE',
        }),
}
